...
public:
    ...
    const T * push(const T * text, const T * end)
    {
        ...
        { // Попытка выделить место в буфере для записи
            d = (T *)__LDREX((volatile uint32_t *)&newDst);
        ...
        } while(__STREX((unsigned long)nd, (volatile uint32_t *)&newDst));
        ...
        { // Попытка обновить dst
            nd = (T *)__LDREX((volatile uint32_t *)&newDst);
            dst = nd;
        } while(__STREX((unsigned long)nd, (volatile uint32_t *)&newDst));
        pull();
        return text + size;
    ...
    inline bool pop(T * result = 0) 
    {
        ...
        do
        {
            s = (T *)__LDREX((volatile uint32_t *)&src);
            if(s == dst) { __CLREX(); return false;}
            if(result) *result = *(s++);
            if(s >= data + SIZE) s -= SIZE;
        } while(__STREX((unsigned long)s, (volatile uint32_t *)&src));
        return true;
    }
